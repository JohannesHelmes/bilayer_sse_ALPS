...
necklace::necklace(const alps::ProcessList& where,const alps::Parameters& p,int node) : alps::scheduler::LatticeMCRun<graph_type>(where,p,node),
...
bool necklace::is_thermalized() const {
        return (sweeps_done>=therm);
}

double necklace::work_done() const {
        return (is_thermalized() ? (sweeps_done-therm)/double(sweeps) :0.);
}


void necklace::diagonal_update() {
	bond_descriptor b;
    //BotCon.clear();
    //vertexlist.clear();
	for (int i=0; i<N; ++i) {
    ...
    for (int p=M1; p<M1+M2; ++p) {
		...
		if (opstring[p]%2==1) {
			...
			if (random_01()<(M2-n2+1)/(Nb*beta*0.5*fact )) { //XY necklace
				--n2;
				opstring[p]=0;
			}
            else {
...
void necklace::save(alps::ODump& dump) const
{
...
void necklace::load(alps::IDump& dump) 
{
...
void necklace::print_copyright(std::ostream & out)
{
